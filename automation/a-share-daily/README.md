...
1. **OpenAI Responses API + Web Search（可选增强）**：仓库配置 `OPENAI_API_KEY` Secret 后启用，用于补充官方来源和过去 24 小时深度研究；
2. **联网数据 + 可审计规则引擎（默认）**：无需模型密钥，依据全市场广度、估值、交易约束、产业证据与账户纪律生成决策；
3. **安全降级**：行情源不完整时仍推送数据状态，但不会在证据不足时给出买入指令。

行情采集按东方财富、新浪、腾讯观察池顺序降级，并同时读取上市公司公告、全球市场和新闻聚合。
